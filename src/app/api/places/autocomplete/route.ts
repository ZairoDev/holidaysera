...
export async function GET(request: NextRequest) {
  try {
    ...
    // `(regions)` returns worldwide geographic results — cities, states, and
    // countries — with NO country restriction, so users can search any place.
    ...
    // Return Google's predictions as-is (no hardcoded country whitelist / reordering).
    const predictions = Array.isArray(data.predictions) ? data.predictions : [];
    const payload = { status: data.status, predictions };
